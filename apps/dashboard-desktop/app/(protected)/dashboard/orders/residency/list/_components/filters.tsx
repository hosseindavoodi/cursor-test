...
import { GetPrivateResidencyOrderListQueries, ResidencyOrderStatus } from "@sana/types/order/residency";
function Filters(props: {
  setFilters: Dispatch<SetStateAction<GetPrivateResidencyOrderListQueries>>;
  filters: GetPrivateResidencyOrderListQueries;
}) {
  const { residency } = getServices();
  const { data } = residency.useResiencyCountries({ queryKey: [{ page: 1, perPage: 1000 }, "RESIDENCY_COUNTRIES"] });
      ...
      <div className="grid grid-cols-5 items-end gap-x-[20px]">
        ...
        <div className="flex flex-col gap-y-1">
          ...
          <Select
            ...
            onValueChange={(value) => props.setFilters((prev) => ({ ...prev, status: value as ResidencyOrderStatus }))}
            dir="rtl"
          >
            <SelectTrigger className="w-full rounded-[8px] h-[44px]">
